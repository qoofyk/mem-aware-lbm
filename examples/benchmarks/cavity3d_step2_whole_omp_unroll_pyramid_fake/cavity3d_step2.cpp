...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        global::argv(8).read(Ty); // number of threads along Y
        global::argv(9).read(Tz); // number of threads along Z

        NyT = Ny / Ty;
        NzT = Nz / Tz;
        pcout << "Each thread's compute domain: " << Nx << 'x' << NyT << 'x' << NzT << '\n';
        // check Nx % ykBlockSize == 0
        // if (Nx % ykBlockSize != 0) throw MyException1();
