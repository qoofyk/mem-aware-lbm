...
namespace plb {
    ...
    plint ykTile;  /* pillarTile. The assumed size, while "actual" memory layout on Length & Width are (ykTile + 2). 
                    E.g. Given computation domain 16*8*16, palabos actual allocated memory is (16+2) * (8+2) * (16+2) 
                    --> Let pillarTile=8, thus Equivalant pillar computation domain is 32 * 8 * 8. Actual allocated memory is (32+2) * (8+2) * (8+2). */
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        global::argv(3).read(ykBlockSize); // parallelepiped block size
